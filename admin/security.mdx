---
title: "Security Architecture of Gestor Financiero"
sidebarTitle: "Security"
description: "Understand the security layers in Gestor Financiero: Argon2id hashing, session cookies, CSRF protection, brute-force lockout, rate limiting, and HTTP security headers."
---
...
`GET`, `HEAD`, and `OPTIONS` requests are exempt. If the check fails, the API returns HTTP 403 with the message `"Token CSRF inválido o ausente."`.
...
In production, if the `HOSTS_PERMITIDOS` environment variable is set, FastAPI's `TrustedHostMiddleware` rejects any request whose `Host` header does not match the configured domain(s). This prevents host-header injection attacks and stops direct-IP access to the backend:
